...
- **BAD:**
  ...
  - Frontend (React/TS) creates an object: `{ userFullName: 'Jane Doe', itemIdentifier: 123 }`
  - A separate function then transforms it to: `{ user_name: 'Jane Doe', item_id: 123 }` before sending to the API.
  - This is wasteful and creates two sources of truth for the object's shape.

- **GOOD:**

  - Frontend (React/TS) creates an object: `{ userName: 'Jane Doe', itemId: 123 }`
  - The Python backend API (e.g., using FastAPI with Pydantic) is designed to accept `userName` and `itemId` directly. If the internal Python convention is `snake_case`, the Pydantic model should use an alias to handle the conversion automatically upon validation, keeping the API contract consistent.python
