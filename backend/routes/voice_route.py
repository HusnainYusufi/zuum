...
async def initialize_transit_chat(stop_id: Optional[int] = None, audio_file: Optional[str] = None):
    try:
        thread_id = random.randint(1, 1000000)
        
        {'response': transit_langgraph_service.run({"messages":[],'running': True, 'stop_id': stop_id}, thread_id), 'thread_id': thread_id}
